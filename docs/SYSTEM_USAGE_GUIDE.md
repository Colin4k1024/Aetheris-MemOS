...
- Qdrant 1.7+
- Neo4j 5.0+ (可选，用于知识图谱)
- Ollama 或任一 OpenAI 兼容端点 (**并非完全可选**)：
  - **嵌入模型为必需**——向量在 LTM 写入与检索的热路径上生成，嵌入后端不可达时这两条路径都会失败（启动时对确定性配置错误 fail-fast，运行时不可达返回 503）。
  - **LLM 摘要可降级**——LLM 不可达时 LTM 写入仍成功、摘要留空并标记 `summaryStatus="pending"` 待事后回填；但 LLM 返回畸形输出仍按 500 上报，不静默降级。
