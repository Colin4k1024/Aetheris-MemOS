...
- **主责角色**：tech-lead
- **分支**：`fix/truthfulness-and-security-remediation`（10 commits，70 文件）
- **收口日期**：2026-08-10
- **当前状态**：`ready-for-review` — **决策已拍板**（见 §2），等待人工 review 后合并
...
4. **7 项「声明 vs 实现」缺口已拍板为「做」**（补齐实现，不删声明），见 §2。
   已按实施成本排序回填 `docs/memory/backlog.md`。
...
## 2. 决策结果（2026-08-10 已拍板）

7 项「声明 vs 实现」缺口，**结论一律为「做」——补齐实现，不删除声明**。

我曾建议其中 4 项走"删/降级"（理由是缩小"声明 > 实现"的表面积），
tech-lead 选择了补齐实现。两条路都能消除失真，区别是成本与产品野心：
删声明立刻消除风险但缩减对外能力，补实现保留能力但需要持续投入。
**既然选择补齐，关键约束是：在实现落地之前，对外表述不得先行。**

完整清单、实施成本排序与推进顺序见 `docs/memory/backlog.md` §A。摘要：

| 顺序 | 项 | 量级 |
|---|---|---|
| 1 | D-5 MCP capability 按主体最小权限 | S |
| 2 | D-4 MCP wasm 沙箱接入（ADR-0004 双平面模型） | M |
| 3 | D-6 open-core 真 feature gating（+ CI 双构建通道防漂移） | M |
| 4 | D-3 A2A（pin 依赖、纳入 CI、让 7 个测试真跑） | M |
| 5 | D-1 gRPC + WebSocket 真实化（ADR-0007） | L |
| 6 | D-2 `kernel/` + `layers/` 接真实存储 | L |
| 7 | D-7 P3 自适应（ADR-0008） | XL |

⚠️ **D-7 有 ADR 自带的前置条件**：ADR-0008 要求先由 eval harness 证明自适应
显著优于静态最优配置才放行，而 `eval_harness.rs::run` 当前硬编码
`passed:true`、`coherence:1.0`、从不调用 scheduler。**第一步必须是把 eval
harness 做成真的**，否则"证明"环节本身是假的。

⚠️ **B 类生产阻塞项（§3）优先级高于本节全部 7 项**：审计不可靠、无告警、
liveness/readiness 不存在、对账扫描器零调用者——这些是上线前的硬门槛，
而 7 项缺口是能力扩展。
