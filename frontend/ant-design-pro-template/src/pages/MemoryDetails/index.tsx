...
const MemoryDetails: React.FC = () => {
  ...
  const stmColumns = [
    ...
    { title: '消息数', dataIndex: 'message_count', key: 'message_count', width: 100 },
    { title: '创建时间', dataIndex: 'created_at', key: 'created_at', width: 180 },
    { title: '最后访问', dataIndex: 'last_accessed_at', key: 'last_accessed_at', width: 180 },
  ];

  const ltmColumns = [
    { title: '条目ID', dataIndex: 'entry_id', key: 'entry_id', width: 200, ellipsis: true },
  ...
  const kgColumns = [
    { title: '实体ID', dataIndex: 'entity_id', key: 'entity_id', width: 200, ellipsis: true },
    { title: '实体名称', dataIndex: 'entity_name', key: 'entity_name', width: 150 },
    {
      title: '实体类型',
  ...
  const mmColumns = [
    ...
    { title: '标题', dataIndex: 'title', key: 'title', width: 200, ellipsis: true },
    {
      ...
      width: 120,
      render: (type: string) => {
        const colorMap: Record<string, string> = { image: 'green', audio: 'orange', video: 'purple', text: 'blue' };
        return <Tag color={colorMap[type] || 'default'}>{type}</Tag>;
      },
    },
    { title: '来源ID', dataIndex: 'source_id', key: 'source_id', width: 150, ellipsis: true },
    { title: '会话ID', dataIndex: 'session_id', key: 'session_id', width: 150, ellipsis: true, render: (id: string) => id || '-' },
  ...
  const tabItems = [
    {
      key: 'stm',
      label: <span><MessageOutlined /> 短期记忆 (STM)</span>,
      children: (
        <ChartCard title="" loading={stmLoading}>
          <Table dataSource={sessions} columns={stmColumns} rowKey="session_id" pagination={{ pageSize: 10 }} size="small" />
        </ChartCard>
      ),
    },
    {
      key: 'ltm',
      label: <span><DatabaseOutlined /> 长期记忆 (LTM)</span>,
      children: (
        <ChartCard title="" loading={ltmLoading}>
          <Table dataSource={ltmEntries} columns={ltmColumns} rowKey="entry_id" pagination={{ pageSize: 10 }} size="small" />
        </ChartCard>
      ),
    },
    {
      key: 'kg',
      label: <span><ApiOutlined /> 知识图谱 (KG)</span>,
      children: (
        <ChartCard title="" loading={kgLoading}>
          <Table dataSource={kgEntities} columns={kgColumns} rowKey="entity_id" pagination={{ pageSize: 10 }} size="small" />
        </ChartCard>
      ),
    },
    {
      key: 'mm',
      label: <span><PictureOutlined /> 多模态记忆 (MM)</span>,
      children: (
        <ChartCard title="" loading={mmLoading}>
          <Table dataSource={mmEntries} columns={mmColumns} rowKey="entry_id" pagination={{ pageSize: 10 }} size="small" />
        </ChartCard>
      ),
    },
  ];
