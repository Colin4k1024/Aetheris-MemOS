...
async function testPage(browser, pageInfo) {
  ...
  try {
    ...
    const antTableCount = await page.$$eval(
      '[class*="ant-table"]',
      (els) => els.length,
    );
    ...
    const statCount = await page.$$eval(
      '[class*="ant-statistic"]',
      (els) => els.length,
    );
    ...
    const loadingSpinners = await page.$$eval(
      '[class*="ant-spin"]',
      (els) => els.length,
    );
    ...
    const errorAlertCount = await page.$$eval(
      '[class*="ant-alert-error"]',
      (els) => els.length,
    );
    if (errorAlertCount > 0) {
      const texts = await page.$$eval('[class*="ant-alert-error"]', (els) =>
        els.map((e) => e.textContent.trim()).slice(0, 2),
      );
    ...
    if (networkErrors.length > 0) {
      const criticalErrors = networkErrors.filter(
        (e) => !e.includes('favicon') && !e.includes('hot-reload'),
      );
      if (criticalErrors.length > 0) {
        console.log(
          `  Network errors: ${criticalErrors.slice(0, 5).join(', ')}`,
        );
        results.warnings.push(
          `Network errors: ${criticalErrors.slice(0, 3).join(', ')}`,
        );
      }
    }
    ...
    if (consoleMessages.length > 0) {
      ...
      if (criticalConsole.length > 0) {
        ...
        criticalConsole.slice(0, 3).forEach((m) => {
    ...
    const functional = [];
    if (cardCount > 0 || statCount > 0)
      functional.push(`cards/stats=${cardCount + statCount}`);
    ...
    console.log(
      `  Functional: ${functional.length > 0 ? functional.join(', ') : 'NONE'}`,
    );
