...
async fn example_write_conversation_history() -> Result<()> {
    ...
    for (role, content) in conversation {
        ...
        let sid = session_id
            .clone()
            .unwrap_or_else(|| "mock_session_123".to_string());
        ...
        info!(
            "写入消息: role={}, message_id={}, session_id={}",
            role, message_id, sid
        );
    }
...
async fn example_extract_key_information(session_id: &str) -> Result<()> {
    ...
    let messages = vec![
        Message {
            role: "user".to_string(),
            content: "你好，我想了解 Rust 编程语言".to_string(),
        },
        Message {
            role: "assistant".to_string(),
            content: "你好！Rust 是一种系统编程语言，注重安全性和性能。".to_string(),
        },
        Message {
            role: "user".to_string(),
        ...
        Message {
            role: "assistant".to_string(),
            content: "Rust 的主要特点包括：内存安全、零成本抽象、并发安全等。".to_string(),
        },
    ];
