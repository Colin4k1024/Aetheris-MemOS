...
impl PerformancePredictionModel {
    fn calculate_synergy_factor(&self, memory_config: &MemoryConfig) -> f64 {
        let active_layers = if memory_config.memory_weights.stm > 0.0 { 1 } else { 0 }
            + if memory_config.memory_weights.ltm > 0.0 { 1 } else { 0 }
            + if memory_config.memory_weights.kg > 0.0 { 1 } else { 0 }
            + if memory_config.memory_weights.mm > 0.0 { 1 } else { 0 };
    ...
    fn calculate_performance_breakdown(
        ...
        PerformanceBreakdown {
            ...
            ltm_contribution: memory_config.memory_weights.ltm
                * self.performance_baselines.ltm.efficiency_gain * 0.48, // 考虑边际递减
            kg_contribution: memory_config.memory_weights.kg
                * self.performance_baselines.kg.efficiency_gain * 0.23, // 考虑边际递减
